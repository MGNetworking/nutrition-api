...
public class DietPlansServiceTest
{
    ...
    public async Task LaunchAsync_ShouldReturnDietResponse_WhenPlanIsValid()
    {
        ...
        var result = await _dietPlansService.LaunchAsync(user.Id, plan.Id);
    ...
    public async Task LaunchAsync_ShouldThrow_WhenPlanNotFound()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() => _dietPlansService.LaunchAsync(user.Id, plan.Id));
    }
    ...
    public async Task LaunchAsync_ShouldThrow_WhenPlanDoesNotBelongToUser()
    {
        ...
        await Assert.ThrowsAsync<ForbiddenException>(() => _dietPlansService.LaunchAsync(otherUserId, plan.Id));
    }
    ...
    public async Task LaunchAsync_ShouldThrow_WhenActiveDietAlreadyExists()
    {
        ...
        await Assert.ThrowsAsync<ConflictException>(() => _dietPlansService.LaunchAsync(user.Id, plan.Id));
    }
    ...
    public async Task LaunchAsync_ShouldThrow_WhenNoWeightEntry()
    {
        ...
        await Assert.ThrowsAsync<UnprocessableException>(() => _dietPlansService.LaunchAsync(user.Id, plan.Id));
    }
}
